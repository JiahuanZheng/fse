...
public class RequestImageImpl implements RequestImage {
	...
	public String backImg(String inputs) {
		long id = MicroTaskOperation.insertMicroTask(inputs, "initial");
		if (id < 0) {
			System.out.println("insert wrong at ImageReceiverImpl 41");
			System.exit(0);
		}
		System.out.println("id is : " + id);
		String backDataXML = NaiveAggregator.aggrerator(id,
				new Date(new Date().getTime() + 1000 * 30 * 1));
		Document doc;
		try {
			doc = DocumentHelper.parseText(backDataXML);
			List<Element> ls = doc.selectNodes("/UIdisplay/TakeImage");

			if (ls != null && ls.size() > 0) {
				Element ele = ls.get(0).element("Value");
				return Image2String(ele.getText());
			}
...
/*
 * @Override public String receiveImg(String inputs, String attachment) {
 * 
 * byte[] buffer = Base64.decodeBase64(attachment); String imgAbsPath = null;
 * long time = (new Date().getTime()); imgAbsPath = byte2Image(buffer,
 * "tmpphoto/ws3" + time + ".jpg");
 * 
 * try { Document doc = DocumentHelper.parseText(inputs);
 * 
 * @SuppressWarnings("unchecked") List<org.dom4j.Element> eles = doc
 * .selectNodes("/task/inputs/input[type='IMG']"); org.dom4j.Element ele =
 * eles.get(0);
 * 
 * Element valEle = ele.element("value"); valEle.setText(imgAbsPath);
 * 
 * System.out.println("doc.asXML()" + doc.asXML());
 * 
 * long id = MicroTaskOperation .insertMicroTask(doc.asXML(), "initial"); if (id
 * < 0) { System.out.println("insert wrong at ImageReceiverImpl 41");
 * System.exit(0); }
 * 
 * System.out.println("id is : " + id);
 * 
 * // new NaiveAggregator().aggrerator(taskXML, deadline) String result =
 * NaiveAggregator.aggrerator(id, new Date(new Date().getTime() + 1000 * 20 *
 * 1)); System.out.println("一分钟时间到了，收到的结果是"+result); //
