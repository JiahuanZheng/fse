...
public class AgentDBOperation {
	...
	/*
	 * "updateWorkerInfo" added by Jiahuan Zheng 20150108 , the function serve as heartbeat mechanism.
	 */
	public static boolean updateWorkerInfo(String guid,String location) throws
	InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
		...
		if (rs.last()) {
			sql = "update agentinfo set location=? ,lastActive=? where guid=?";
			ps = conn.prepareStatement(sql);
			ps.setString(1, location);
			ps.setString(2, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
			ps.setString(3, guid);
			if (ps.executeUpdate() == 0) {
				System.out.println("Error occured in updateWorkerInfo().");
				return false;
			}
//			System.out.println("update agent info succeed!");
	...
	public static boolean register(String guid, String address, String capacity)
		...
		if (rs.last()) {
			...
			ps.setString(4, "");
			ps.setString(6, capacity);// added by Jiahuan Zheng 20150108
